...
app.get('/api/usernameexists/:username', function (req, res) {
    ...
    dbConn.query("SELECT EXISTS(SELECT * FROM users WHERE username='" + check_username + "') AS Result", (err, res2)=> {
        console.log(err|| res2);
        console.log(res2[0].Result);
        if (res2[0].Result) {
            apiResult= { error: false, message: 'Username already exists, please select a different username.'}; 
        }
        else {
            apiResult = { error: false, message: 'Username has not been used yet.' };
        }
        res.json(apiResult);
    });
    ...
    dbConn.query(sql_check, username, function(error, results, fields) {
        if(error) throw error;
            ...
            if (password == results[0].password){
                ...
                return res.send({ error: false, message: 'Wrong password, please try again.'});
            }
        }
    })
});
...
app.post('/api/Physiotherapist/addpatient', function (req, res) {
    ...
    let sql_create = 'CREATE TABLE IF NOT EXISTS PhysiotherapistAndPatient (patient_username VARCHAR(255) NOT NULL, physiotherapist_username VARCHAR(255) NOT NULL, Startdate DATE, LastVisitDate DATE, STATUS TINYINT(1), PRIMARY KEY (patient_username))';
    dbConn.query(sql_create, patient,function (error, results, fields) {
          if (error) throw error;
           })
    
    dbConn.query('INSERT INTO PhysiotherapistAndPatient SET ? ', patient, function (error, results, fields) {
            if (error) throw error;
            return res.send({ error: false, data: results, message: 'Patient has been added successfully.' });
        });
    });
...
app.post('/api/Caretaker/addpatient', function (req, res) {
    ...
    let sql_create = 'CREATE TABLE IF NOT EXISTS CaretakerAndPatient (patient_username VARCHAR(255) NOT NULL, caretaker_username VARCHAR(255) NOT NULL, PRIMARY KEY (patient_username))';
    dbConn.query(sql_create, patient, function (error, results, fields) {
         if (error) throw error;
            })
    dbConn.query('INSERT INTO CaretakerAndPatient SET ?', patient, function (error, results, fields) {
        if (error) throw error;
        return res.send({ error: false, data: results, message: 'Patient has been added successfully.' });
            })
});
...
app.post('/api/Physiotherapist/ViewPatientsDetails', function (req,res) {
    ...
    dbConn.query('SELECT * FROM patientdetails WHERE patient_username=?', username, function(error, results, fields) {
        if(error) throw error;
        return res.send({ error: false, data: results[0], message: 'patient details'});
    });
...
app.post('/api/Physiotherapist/viewPatients', function (req, res) {
    ...
    if (!physiotherapist_username) {
     return res.status(400).send({ error: true, message: 'Please provide your username' });
    }

    let patients = "SELECT PhysiotherapistAndPatient.patient_username, PhysiotherapistAndPatient.Startdate, PhysiotherapistAndPatient.LastVisitDate, users.name FROM PhysiotherapistAndPatient INNER JOIN users ON PhysiotherapistAndPatient.patient_username = users.username WHERE PhysiotherapistAndPatient.physiotherapist_username ='" + physiotherapist_username +"'";
...
app.post('/api/Physiotherapist/viewAllMyPatientsDetails', function (req,res) {
    ...
    let patient_details = "SELECT PhysiotherapistAndPatient.patient_username, patientdetails.ActiveExercise, patientdetails.ActiveExercise, patientdetails.ExerciseID, patientdetails.TotalExerciseCount, patientdetails.WeeklySchedule, patientdetails.TotalExerciseCompleted, PhysiotherapistAndPatient.LastVisitDate FROM PhysiotherapistAndPatient LEFT JOIN patientdetails ON PhysiotherapistAndPatient.patient_username = patientdetails.patient_username WHERE PhysiotherapistAndPatient.physiotherapist_username ='" + physiotherapist_username + "'ORDER BY PhysiotherapistAndPatient.patient_username";
    
    dbConn.query(patient_details, function(error, results, fields){
        if (error) throw error;
        return res.send({error: false, data:results, message: 'patient details'})
    });
});
...
app.post('/api/Physiotherapist/UpdateExercise', function(req,res) {
    ...
    if (!patient_username) {
        return res.status(400).send({ error: true, message: 'Please insert username' });
       }
    if (!ExerciseID) {
        return res.status(400).send({ error: true, message: 'Please insert ExerciseID' });
    }
    if (!ExerciseDate) {
        return res.status(400).send({ error: true, message: 'Please insert dates' });
       }
    if (!TotalExerciseCount) {
        return res.status(400).send({ error: true, message: 'Please insert exercise count' });
       }
    
    let sql_update = "UPDATE patientdetails " + "SET ExerciseID='" + ExerciseID + "', ExerciseDate='" + ExerciseDate + "', TotalExerciseCount='" + TotalExerciseCount + "'" + " WHERE patient_username='" + patient_username +"'"
...
app.post('/api/Patient/viewExercise', function(req,res) {
    ...
    let sql_show = "SELECT ExerciseDate, TotalExerciseCount-TotalExerciseCompleted AS 'TotalExercisesLeft', ExerciseID FROM patientdetails WHERE patient_username= '" + username+"';"

    dbConn.query(sql_show, function(error, results, fields) {
        if (error) throw error;
        return res.send({error: false, data: results, message: 'Here are your exercises!'})
    })
})
...
app.post('/api/Patient/completeExercise', function(req,res) {
    ...
    let sql_increment = "UPDATE patientdetails " + "SET TotalExerciseCompleted = TotalExerciseCompleted + 1" + " WHERE patient_username= '" + patient_username + "' AND ExerciseID= '" + ExerciseID + "' ;"
...
app.post('/api/Patient/viewExerciseCompletion', function(req,res) {
    ...
    let sql_complete = "SELECT ExerciseID, TotalExerciseCompleted, TotalExerciseCount-TotalExerciseCompleted AS 'TotalExercisesLeft', (TotalExerciseCompleted/TotalExerciseCount)*100 AS 'ExerciseProgress' " + "FROM patientdetails " + "WHERE patient_username = '"+ patient_username +"';"
    dbConn.query(sql_complete, function(error, results, fields) {
        if(error) throw error;
        return res.send({error: false, data: results, message: "Here is your exercise completion progress!"})
    })
})
...
app.post('/api/Patient/viewPhysiotherapist', function(req,res) {
    ...
    let sql_view = "SELECT PhysiotherapistAndPatient.physiotherapist_username, users.name " + "FROM users INNER JOIN PhysiotherapistAndPatient ON PhysiotherapistAndPatient.physiotherapist_username = users.username " + "WHERE PhysiotherapistAndPatient.patient_username = '" + patient_username + "';"
    dbConn.query(sql_view, function(error, results, fields) {
        if(error) throw error;
        return res.send({error: false, data: results, message: "Physiotherapist"})
    })
})
...
app.post('/api/Patient/viewCaretaker', function(req,res) {
    ...
    let sql_view = "SELECT CaretakerAndPatient.caretaker_username, users.name " + "FROM users INNER JOIN CaretakerAndPatient ON CaretakerAndPatient.caretaker_username = users.username " + "WHERE CaretakerAndPatient.patient_username = '" + patient_username + "';"
    dbConn.query(sql_view, function(error, results, fields) {
        if(error) throw error;
...
app.post('/api/Caretaker/viewPatient', function (req, res) {
    ...
    let patients = "SELECT CaretakerAndPatient.patient_username, users.name FROM CaretakerAndPatient INNER JOIN users ON CaretakerAndPatient.patient_username = users.username WHERE PhysiotherapistAndPatient.caretaker_username ='" + caretaker_username +"'";
...
app.post('/api/Caretaker/viewExercise', function (req,res) {
    ...
    dbConn.query(sql_complete, function(error, results, fields) {
        if(error) throw error;
        return res.send({error: false, data: results, message: "Here is your patient's exercise completion progress!"})
    })
})
